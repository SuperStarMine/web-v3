...
{#if imageExtensionsShort.includes('svg')}
  <picture on:click={click} {title}>
    <img src="{imageDirectory}{imageId}.svg" {alt} {width} {height} loading={loadLazy ? 'lazy' : 'eager'}>
  </picture>
{:else}
  <picture on:click={click} {title}>
    ...
    {/each}
    <img {sizes} srcset="{resolveSrcsets(imageDirectory, imageExtensionsShort, imageSizes, imageId, loading, tinyImageExtensionsShort, tinyImageSize)[getSafeImageExtensionIndex(imageExtensionsShort.filter(v => v != 'svg'))]}" {alt} {width} {height} loading={loadLazy ? 'lazy' : 'eager'}>
  </picture>
{/if}
